...
// QueryPatientCoveredPerson chains the current query on the Patient_CoveredPerson edge.
func (pq *PatientQuery) QueryPatientCoveredPerson() *CoveredPersonQuery {
	query := &CoveredPersonQuery{config: pq.config}
...
//  WithPatientCoveredPerson tells the query-builder to eager-loads the nodes that are connected to
// the "Patient_CoveredPerson" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PatientQuery) WithPatientCoveredPerson(opts ...func(*CoveredPersonQuery)) *PatientQuery {
	query := &CoveredPersonQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPatientCoveredPerson = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withPatientCoveredPerson; query != nil {
		...
		query.Where(predicate.CoveredPerson(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.PatientCoveredPersonColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.PatientCoveredPerson = append(node.Edges.PatientCoveredPerson, n)
		}
	}
